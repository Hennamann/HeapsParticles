...
/**
    Plist particle loader
**/
class PlistParticleLoader {
    public static function Load(configFile : String, texture : Tile) : Particles2D {        
        var root = Xml.parse(configFile).firstElement().firstElement();
        ...
        ps.blendMode = ParticleLoader.GetBlendMode(map["blendMode"].asInt());        
